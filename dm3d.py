...
class AttentionBlock(layers.Layer):
    ...
    def call(self, inputs):
        ...
        attn_score = tf.reshape(attn_score,
                                [batch_size, height, width, depth, height * width*depth])
...
class CrossAttentionBlock(layers.Layer):
    ...
    def __init__(self, units, num_heads, **kwargs):
        ...
        self.proj_in = layers.Conv3D(units, 1)  # Is it necessary?
        self.proj_out = layers.Conv3D(units, 1)  # Is it necessary?
        ...
        # , kernel_initializer=kernel_init(1.0))
        self.depth = layers.Dense(units)
        self.proj = keras.Sequential([layers.Dense(units*4),  # kernel_initializer=kernel_init(0.0)),
                                      layers.ReLU(),
                                      layers.Dense(units)])
    ...
    def reshape_head_to_batch_dim(self, inputs):
        ...
        if self.num_heads > 1:
            inputs = tf.reshape(
                inputs, (batch_size, seq_len, self.num_heads, channels//self.num_heads))
            inputs = tf.transpose(inputs, (0, 2, 1, 3))
            inputs = tf.reshape(
                inputs, (batch_size*self.num_heads, seq_len, channels//self.num_heads))
        return inputs
    ...
    def reshape_batch_dim_to_head(self, inputs):
        ...
        if self.num_heads > 1:
            inputs = tf.reshape(
                inputs, (batch_size//self.num_heads, self.num_heads, seq_len, channels))
            inputs = tf.transpose(inputs, (0, 2, 1, 3))
            inputs = tf.reshape(
                inputs, (batch_size//self.num_heads, seq_len, channels*self.num_heads))
        return inputs
    ...
    def attention(self, inputs, context=None):
        scale = tf.cast(self.units, tf.float32) ** (-0.5)
        batch_size, height, width, depth = inputs.shape[:-1]

        q = self.query(inputs)
        k = self.key(inputs) if context is None else self.key(context)
        v = self.value(inputs) if context is None else self.key(context)

        q, k, v = self.reshape_head_to_batch_dim(
            q), self.reshape_head_to_batch_dim(k), self.reshape_head_to_batch_dim(v)
...
def TimeMLP(units, activation_fn=keras.activations.swish):
    def apply(inputs):
        temb = layers.Dense(
            units, activation=activation_fn, kernel_initializer=kernel_init(
                1.0)
        )(inputs)
...
def build_model(
    ...
    context_input = keras.Input(shape=(1, context_dim), name='context_input')
    ...
    temb = TimeMLP(units=first_conv_channels * 4,
    ...
    for i in range(len(widths)):
        for _ in range(num_res_blocks):
            ...
            #     x=CrossAttentionBlock()(x,context)
            skips.append(x)
    ...
    x = layers.BatchNormalization()(x)
    x = activation_fn(x)
    x = layers.Conv3D(img_channels, (3, 3, 3), padding="same",
...
class DiffusionModel(keras.Model):
    ...
    def __init__(self, latent_size, num_embed, latent_channels, vqvae_load_ckpt, args):
        ...
        self.network = build_model(latent_size,
                                   latent_channels,
                                   widths=[64, 128, 256],
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape() as tape:
            latents = self.quantizer(self.encoder(images))

            # 3. Sample random noise to be added to the images in the batch
            noise = tf.random.normal(
                shape=tf.shape(latents), dtype=images.dtype)

            # 4. Diffuse the images with noise
            sqb = tf.reshape(tf.gather(self.b.sqrt_alpha_bar, t), [
                             batch_size, 1, 1, 1, 1])
            osqb = tf.reshape(tf.gather(self.b.sqrt_one_minus_alpha_bar, t), [
                              batch_size, 1, 1, 1, 1])
            ...
            loss_reduction_factor = self.global_bs*self.lc*self.lc*self.lc*self.lc*1.0
    ...
    def sample(self, x_t, pred_noise, curr_time_step, shape):
        b = tf.reshape(tf.gather(self.b.beta, curr_time_step),
                       [shape[0], 1, 1, 1, 1])
        sqa = tf.reshape(tf.gather(self.b.sqrt_alpha, curr_time_step), [
                         shape[0], 1, 1, 1, 1])
        ab = tf.reshape(tf.gather(self.b.alpha_bar, curr_time_step), [
                        shape[0], 1, 1, 1, 1])
        ab_prev = tf.reshape(tf.gather(self.b.alpha_bar_prev, curr_time_step), [
                             shape[0], 1, 1, 1, 1])
        sqab = tf.reshape(tf.gather(self.b.sqrt_alpha_bar,
                          curr_time_step), [shape[0], 1, 1, 1, 1])
        sqab_prev = tf.reshape(tf.gather(self.b.sqrt_alpha_bar_prev, curr_time_step), [
                               shape[0], 1, 1, 1, 1])
        sq1ab = tf.reshape(tf.gather(self.b.sqrt_one_minus_alpha_bar, curr_time_step), [
                           shape[0], 1, 1, 1, 1])
        x_0 = (x_t - sq1ab*pred_noise)/sqab
        # x_0 = pred_noise

        posterior_mean = (b*sqab_prev/(1-ab))*x_0 + \
            ((1-ab_prev)*sqa/(1-ab))*x_t

        posterior_log_variance = (1-ab_prev)*b/(1-ab)

        return posterior_mean, posterior_log_variance

    def generate(self, shape=(1, 16, 16, 16, 16), last_step=0):
        ...
        for i in range(self.timesteps-1, last_step-1, -1):
            ...
            pred_noise = self.network([samples, curr_time_step])
