...
from networks.callbacks import DelayedReduceLROnPlateau
from dataset_utils import create_dataset, load_dataset, get_dataset_list
from networks.vqgan_attn_cp import VQGAN, WandbImageCallback, EpochCounterCallback
...
def build_and_compile_model(channel_list, num_embedding, embedding_dim, args, strategy):
    with strategy.scope():
        ...
        downsample_parameters = [(1, 4, 1, "same") for _ in channel_list]  
        upsample_parameters = [(1, 4, 1, "same", 0) for _ in channel_list]  
        ...
        print(m.summary())
        vqvae_optimizer=keras.optimizers.Adam(learning_rate=args.lr, beta_1=0.5, beta_2=0.9)
        model.compile(vqvae_optimizer=vqvae_optimizer,
                      discriminator_optimizer=keras.optimizers.Adam(learning_rate=args.lr, beta_1=0.5, beta_2=0.9))

    return model, vqvae_optimizer


def run_experiment(args, strategy, train_dataset, val_dataset):
    run_name = f"{args.channel_list}|{args.num_embedding}x{args.embedding_dim}|{args.disc_threshold}|{args.disc_loss_fn}|{args.dataset}" 
    ...
    model, vqvae_optimizer = build_and_compile_model(channels, num_embedding, embedding_dim, args, strategy)
    ...
    delayed_reduce_lr = DelayedReduceLROnPlateau(
        monitor="val_loss",
        factor=0.002,
        patience=8,
        min_lr=1e-6,
        verbose=1,
        start_epoch=args.disc_threshold)
    
    wandbImage = WandbImageCallback(model, val_dataset, log_freq=10)
    epoch_counter = EpochCounterCallback(model)
    if args.test_run:
        callbacks = [delayed_reduce_lr, WandbCallback(save_model=False), wandbImage, epoch_counter]
    else:
        callbacks = [WandbCallback(save_model=False), wandbImage, epoch_counter, delayed_reduce_lr]
