...
if __name__=='__main__':
    ...
    print('model path', vgg_ckpt_fn)
    model = learned_perceptual_metric_model(128, vgg_ckpt_fn, lin_ckpt_fn)
    print('weights loaded')
    image_shape = (6, 128, 128, 3)
    image0 = tf.convert_to_tensor(np.random.random(image_shape), dtype=tf.float32)
    image1 = tf.convert_to_tensor(np.random.random(image_shape), dtype=tf.float32)

    dist = model([image0, image1])
