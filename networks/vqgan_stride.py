...
class VQVAEResidualUnit(keras.Model):
    def __init__(self, input_channels, num_res_channels, act_fn, dropout=None):
        ...
        # if input_channels == 2:
        #     self.conv2.add(tf.keras.layers.GroupNormalization(groups=1, epsilon=1e-6))
        # else:
        ...
        for i in range(len(self.num_channels)):
            if i == 0:
                for _ in range(self.num_res_layers):
                    self.blocks.add(VQVAEResidualUnit(input_channels=self.in_channels, 
                                                      num_res_channels=self.num_res_channels[i], 
                                                      act_fn='prelu'))
            else:
                for _ in range(self.num_res_layers):
                    self.blocks.add(VQVAEResidualUnit(input_channels=self.num_channels[i-1], 
                                                      num_res_channels=self.num_res_channels[i], 
                                                      act_fn='prelu'))
            ...
            self.blocks.add(layers.AveragePooling3D(pool_size=(2, 2, 2)))  # Downsampling
            ...
            if i == len(self.num_channels)-1:
                self.blocks.add(tf.keras.layers.GroupNormalization(self.num_channels[i], epsilon=1e-6))
...
class Decoder(keras.Model):
    ...
    def __init__(
        ...
        for i in range(len(self.num_channels)):
            ...
            self.blocks.append(layers.Conv3D(out, 
                                             self.upsample_parameters[i][1], 
            ...
            self.blocks.append(layers.UpSampling3D(size=(2,2,2)))
            # self.blocks.append(tf.keras.layers.BatchNormalization())
            if out < 32:
                self.blocks.append(tf.keras.layers.GroupNormalization(int(out/2), epsilon=1e-6))
            # else:
            #     self.blocks.append(tf.keras.layers.GroupNormalization(out, epsilon=1e-6))
