...
class CrossAttentionBlock(layers.Layer):
    ...
    def reshape_head_to_batch_dim(self, inputs):
        ...
        if self.num_heads > 1:
            inputs = tf.reshape(
                inputs,
                (batch_size, seq_len, self.num_heads, channels // self.num_heads),
            )
            inputs = tf.transpose(inputs, (0, 2, 1, 3))
            inputs = tf.reshape(
                inputs,
                (batch_size * self.num_heads, seq_len, channels // self.num_heads),
            )
        return inputs
    ...
    def reshape_batch_dim_to_head(self, inputs):
        ...
        if self.num_heads > 1:
            inputs = tf.reshape(
                inputs,
                (batch_size // self.num_heads, self.num_heads, seq_len, channels),
            )
            inputs = tf.transpose(inputs, (0, 2, 1, 3))
            inputs = tf.reshape(
                inputs,
